...
air = ncout.createVariable('air', np.float64, ('time','lon','lat'), fill_value=np.nan)
air.standard_name = 'air_temperature'
air.long_name = 'air temperature'
air.units = 'Celsius Degree'
air.axis = 'K'
print(air)
...
lat[:] = x
air[:,:,:] = zz
print("-- Wrote data, air.shape is now ", air.shape)
# read data back from variable (by slicing it), print min and max
print("-- Min/Max values:", air[:,:,:].min(), air[:,:,:].max())

#Add global attributes
ncout.type    = 'Meteology File'
ncout.title   = 'Air temperature'
