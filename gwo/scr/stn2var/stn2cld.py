...
python stn2map.py 100 './ajiro/1991-2017_temp.csv' \
'./chiba/1991-2017_temp.csv' \
'./chichibu/1991-2017_temp.csv' \
'./choshi/1991-2017_temp.csv' \
'./irozaki/1991-2017_temp.csv' \
...
cld = ncout.createVariable('cld', np.float64, ('time','lon','lat'), fill_value=np.nan)
cld.standard_name = 'cloud_coverage'
cld.long_name = 'cloud coverage'
cld.units = '-'
cld.axis = '-'
print(cld)
...
lat[:] = x
cld[:,:,:] = zz
print("-- Wrote data, cld.shape is now ", cld.shape)
# read data back from variable (by slicing it), print min and max
print("-- Min/Max values:", cld[:,:,:].min(), cld[:,:,:].max())

#Add global attributes
ncout.type    = 'Meteology File'
ncout.title   = 'Cloud coverage'
...
#plt.colorbar()  # draw colorbar
## plot data points.
#plt.scatter(lat, lon, c='m', marker='o', s=6, zorder=10)
#plt.xlim(139.145, 140.390)
#plt.ylim(34.73, 35.88)
#plt.title('griddata test (%d points)' % len(points))
#plt.show()