...
#            st_vl_st1.append(float(stnv.iloc[j,1]))
#            st_vl_st1.append(float(st_vl_st1[-1]))
            st_vl_st2.append(float(stnv.iloc[j,2]))
            st_vl_st2.append(float(st_vl_st2[-1]))
        else:
#            st_vl_st1.append(float(stnv.iloc[j,1]))
            st_vl_st2.append(float(stnv.iloc[j,2]))
#    st_vl_st1.append(float(stnv.iloc[-1,1]))
    st_vl_st2.append(float(stnv.iloc[-1,2]))
#    st_vl1.append(st_vl_st1)
    st_vl2.append(st_vl_st2)
#values1 = np.array(st_vl1)
values2 = np.array(st_vl2)
...
#for k in range(480):
#    grid_z01 = griddata(points, values1[:,k], (xv, yv), method='cubic')
#    zv1.append(grid_z01)
    grid_z02 = griddata(points, values2[:,k], (xv, yv), method='cubic')
    zv2.append(grid_z02)
...
#uwnd = ncout.createVariable('uwnd', np.float64, ('time','lon','lat'), fill_value=np.nan)
#uwnd.standard_name = 'Eastward Wind Speed'
#uwnd.long_name = 'Eastward Wind Speed'
#uwnd.units = 'm/s'
#uwnd.axis = 'U'
#print(uwnd)

vwnd = ncout.createVariable('vwnd', np.float64, ('time','lon','lat'), fill_value=np.nan)
vwnd.standard_name = 'Northward Wind Speed'
vwnd.long_name = 'Northward Wind Speed'
vwnd.units = 'm/s'
vwnd.axis = 'V'
print(vwnd)
...
#uwnd[:,:,:] = zz1
vwnd[:,:,:] = zz2
print("-- Wrote data, uwnd.shape is now ", vwnd.shape)
# read data back from variable (by slicing it), print min and max
print("-- Min/Max values:", vwnd[:,:,:].min(), vwnd[:,:,:].max())

#Add global attributes
ncout.type    = 'Meteology File'
ncout.title   = 'Wind speed V'
